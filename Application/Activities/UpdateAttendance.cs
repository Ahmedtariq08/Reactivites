        ...
        public class Command : IRequest<Result<Unit>>
        {
            ...
            public class Handler : IRequestHandler<Command, Result<Unit>>
            {
                ...
                public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
                {
                    ...
                    //if activity is nout found return 404
                    ...
                    //User is host of this activity, then it should not be cancelled
                    if (attendance != null && hostUsername == user.UserName)
                        activity.IsCancelled = !activity.IsCancelled;

                    //If user is not found in the attendees then remove them
                    if (attendance != null && hostUsername != user.UserName)
                        activity.Attendees.Remove(attendance);

                    //If this user is not found in the attendees of this activity, then add the user
                    if (attendance == null)
                    {
